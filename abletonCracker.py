...
def replace_signkey_in_file(file_path, old_signkey, new_signkey, undo: bool = False, silent: bool = False):
...
def main():
    ...
    parser.add_argument('--silent', action='store_true', help='Run in silent mode (suppress non-essential output)')
    parser.add_argument('--no-output-file', action='store_true', help='Skip generating the authorization file')
    ...
    if args.help:
        ...
        print("  --silent                    Run in silent mode (suppress non-essential output)")
        print("  --no-output-file            Skip generating the authorization file")
...
(____  /___  /____/\___  >__|  \____/|___|  /\______  /|__|  (____  /\___  >__|_ \\___  >__|   
   ...
   """ + RESET)
    ...
    if args.undo and not args.silent:
    ...
    if file_path.lower() == "auto":
        ...
        try:
            if not args.silent:
                ...
                # In silent mode, use the first installation found
                selection = 0
    ...
    # Skip authorization file generation in undo mode or if --no-output-file is specified
    if not args.undo and not args.no_output_file:
            ...
            if not data_dirs:
                ...
                try:
                    if not args.silent:
                        ...
                        # In silent mode, use the first directory found
                        selection = 0
    ...
    try:
        replace_signkey_in_file(file_path, old_signkey, new_signkey, args.undo, args.silent)
        if not args.silent:
            print(GREEN + "\nPatch completed successfully!" + RESET)
        
        # Only show the authorization file instructions and folder opening in normal mode
        if not args.undo and not args.no_output_file and not args.silent:
